...
class TestCVLayerIntegration:
    ...
    def test_extracted_element_to_reasoning_input_conversion(self, sample_physics_element):
        ...
        assert reasoning_input.get_element_text() == 'Force Diagram: mg = 50N ↓, N = 50N ↑, f = 10N ←'
        assert reasoning_input.page_metadata['subject_hint'] == 'physics'
        assert isinstance(reasoning_input.extracted_element, dict)

    def test_spatial_context_extraction(self, sample_physics_element, sample_surrounding_elements):
    ...
    def test_subject_detection_with_cv_data(self, sample_chemistry_element):
        ...
        context_text = 'Chemical reactions and stoichiometry ' + sample_chemistry_element.ocr_text
    ...
    def test_pedagogical_alt_text_generation(self, sample_physics_element):
        ...
        detected_subject = context_processor.detect_subject_area(
            'force diagram physics vectors',
            {'subject_hint': 'physics'}
        )
        
        # Generate alt-text
        mock_llm_response = "Physics force diagram showing three force vectors in equilibrium."
    ...
    def test_complete_pipeline_physics(self, sample_physics_element, sample_surrounding_elements):
        ...
        spatial_context = context_processor.extract_spatial_context(
            ...
            reasoning_input.surrounding_elements
        )
        
        detected_subject = context_processor.detect_subject_area(
            spatial_context.preceding_text + ' ' + spatial_context.following_text + ' ' + reasoning_input.get_element_text(),
    ...
    def test_complete_pipeline_chemistry(self, sample_chemistry_element):
        ...
        detected_subject = context_processor.detect_subject_area(
            'chemical reaction methane combustion ' + reasoning_input.get_element_text(),
            reasoning_input.page_metadata
        )
        ...
        assert 'chemistry' in alt_text_result.alt_text.lower() or 'chemical' in alt_text_result.alt_text.lower()
        assert 'reaction' in alt_text_result.alt_text.lower() or 'equation' in alt_text_result.alt_text.lower()
    ...
    def test_semantic_reasoner_with_cv_data(self, mock_openai, sample_physics_element):
        ...
        mock_response.choices[0].message.content = "Physics force diagram showing equilibrium forces."
    ...
    def test_data_format_compatibility(self):
        ...
        required_fields = ['element_id', 'classification', 'bounding_box', 'ocr_text', 'html_tag']
    ...
    def test_performance_with_multiple_elements(self):
        ...
        for element in elements:
            reasoning_input = ReasoningInput.from_cv_output(
                ...
                surrounding_elements=[e.model_dump() for e in elements if e != element][:5]
            )
