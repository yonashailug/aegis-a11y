...
from reasoning_agent.human_validator import (
    HumanValidator,
    ReviewAction,
    ReviewerRole,
    ReviewFeedback,
)
from reasoning_agent.quality_assessor import QualityAssessor
from reasoning_agent.schemas import (
    ConfidenceLevel,
    ReasoningOutput,
    SubjectArea,
)
from reasoning_agent.verifier import (
...
class TestVerifierIntegration:
    ...
    def test_deterministic_verifier_pass(
        self, sample_reasoning_output, sample_element, spatial_context
    ):
    ...
    def test_deterministic_verifier_poor_quality(
        self, poor_quality_output, sample_element, spatial_context
    ):
        """Test verifier with poor-quality output (should identify quality issues)."""
        verifier = DeterministicVerifier()

        result = verifier.verify_reasoning_output(
            poor_quality_output, sample_element, {"spatial_context": spatial_context}
        )
        ...
        if len(result.issues) > 0:
            generic_issues = [
                i for i in result.issues if "generic" in i.description.lower()
            ]
    ...
    def test_quality_assessor_metrics(
        self, sample_reasoning_output, sample_element, spatial_context
    ):
        ...
        metrics = assessor.assess_alt_text_quality(
            sample_reasoning_output, sample_element, spatial_context
        )
    ...
    def test_quality_assessor_subject_relevance(
        self, sample_reasoning_output, sample_element, spatial_context
    ):
    ...
    def test_human_validator_session_creation(
        self, sample_reasoning_output, spatial_context
    ):
        ...
        session = validator.create_review_session(
            sample_reasoning_output, quality_metrics, verification_result
        )
    ...
    def test_human_validator_review_submission(
        self, sample_reasoning_output, spatial_context
    ):
    ...
    def test_human_validator_queue_management(
        self, sample_reasoning_output, spatial_context
    ):
        ...
        queue_item = next(
            item for item in educator_queue if item["session_id"] == session.session_id
        )
    ...
    def test_complete_pipeline_integration(
        ...
        review_result = validator.submit_review(
            review_session.session_id, educator_review
        )
        ...
        assert final_report["verification_summary"]["overall_status"] in [
            "pass",
            "warning",
        ]
    ...
    def test_error_handling_and_edge_cases(self):
        ...
        result = verifier.verify_reasoning_output(
            minimal_output, {}, {}  # Empty element  # Empty context
        )
