...
class QualityAssessor:
    ...
    def assess_alt_text_quality(
        ...
        pedagogical_score = self._assess_pedagogical_alignment(
            alt_text, subject_area, spatial_context
        )
        ...
        conceptual_score = self._assess_conceptual_accuracy(
            alt_text, subject_area, original_element
        )
        learning_alignment = self._assess_learning_objective_alignment(
            alt_text, spatial_context
        )
    ...
    def _assess_pedagogical_alignment(
        self, alt_text: str, subject_area: SubjectArea, spatial_context: dict[str, Any]
    ) -> float:
        ...
        has_pedagogical_language = any(
            indicator in alt_text for indicator in pedagogical_indicators
        )
        ...
        learning_focus = sum(
            1 for indicator in learning_indicators if indicator in alt_text
        )
        ...
        if subject_area in self.subject_vocabularies:
            ...
            concept_mentions = sum(
                1 for concept in vocab["concepts"] if concept in alt_text
            )
            relationship_mentions = sum(
                1 for rel in vocab["relationships"] if rel in alt_text
            )
    ...
    def _assess_structural_correctness(
        self, output: ReasoningOutput, original_element: dict[str, Any]
    ) -> float:
    ...
    def _assess_readability(self, alt_text: str) -> float:
        ...
        structure_score = sum(
            1 for indicator in structure_indicators if indicator in alt_text
        )
    ...
    def _assess_subject_relevance(
        self, alt_text: str, subject_area: SubjectArea
    ) -> float:
        """Assess relevance to detected subject area."""

        if (
            subject_area == SubjectArea.UNKNOWN
            or subject_area not in self.subject_vocabularies
        ):
        ...
        total_terms = (
            len(vocab["concepts"])
            + len(vocab["relationships"])
            + len(vocab["measurements"])
        )

        # Count subject-specific terms
        concept_matches = sum(1 for term in vocab["concepts"] if term in alt_text)
        relationship_matches = sum(
            1 for term in vocab["relationships"] if term in alt_text
        )
        measurement_matches = sum(
            1 for term in vocab["measurements"] if term in alt_text
        )
    ...
    def _assess_udl_compliance(self, alt_text: str, output: ReasoningOutput) -> float:
        ...
        if (
            hasattr(output, "udl_guidelines_applied")
            and len(output.udl_guidelines_applied) > 0
        ):
            base_score *= 1.2

        return min(1.0, base_score)

    def _assess_vocabulary_appropriateness(
        self, alt_text: str, subject_area: SubjectArea
    ) -> float:
        ...
        has_academic_vocab = any(
            indicator in alt_text for indicator in academic_indicators
        )
    ...
    def _assess_conceptual_accuracy(
        self, alt_text: str, subject_area: SubjectArea, original_element: dict[str, Any]
    ) -> float:
    ...
    def _assess_learning_objective_alignment(
        self, alt_text: str, spatial_context: dict[str, Any]
    ) -> float:
    ...
    def _identify_accessibility_features(
        self, alt_text: str, output: ReasoningOutput
    ) -> list[str]:
        ...
        if output.detected_subject_area != SubjectArea.UNKNOWN:
            features.append(
                f"Subject-specific vocabulary ({output.detected_subject_area.value})"
            )
    ...
    def _generate_improvement_suggestions(
        ...
        if pedagogical_score < 3.5:
            suggestions.append(
                "Enhance pedagogical focus with learning-oriented language"
            )
        ...
        if len(alt_text) > 250:
            suggestions.append(
                "Consider condensing while maintaining key educational points"
            )
