...
from .prompt_templates import GENERAL_TEMPLATES, HUMANITIES_TEMPLATES, STEM_TEMPLATES
from .schemas import (
    ConfidenceLevel,
    LLMRequest,
    LLMResponse,
...
class SemanticReasoner:
    ...
    def process_element(self, input_data: ReasoningInput) -> ReasoningOutput:
        ...
        try:
            ...
            spatial_context = self.context_processor.extract_spatial_context(
                input_data.extracted_element, input_data.surrounding_elements
            )
            ...
            llm_request = self._build_llm_request(
                input_data, spatial_context, detected_subject
            )
            ...
            processing_warnings.append(f"Error in main processing: {e!s}")
            fallback_used = True

            return self._fallback_processing(
                input_data, start_time, processing_warnings, str(e)
            )

    def _build_llm_request(
        self, input_data: ReasoningInput, spatial_context, detected_subject
    ) -> LLMRequest:
    ...
    def _get_prompt_template(self, subject_area: SubjectArea) -> str:
        ...
        elif subject_area in [
            SubjectArea.HISTORY,
            SubjectArea.LITERATURE,
            SubjectArea.SOCIAL_STUDIES,
        ]:
            return HUMANITIES_TEMPLATES.get(
                subject_area.value, GENERAL_TEMPLATES["default"]
            )
        else:
    ...
    def _assess_quality(self, alt_text_result, detected_subject) -> float:
        ...
        keyword_matches = sum(
            1 for kw in keywords if kw.lower() in alt_text_result.alt_text.lower()
        )
        ...
        if any(
            indicator in alt_text_result.alt_text.lower()
            for indicator in pedagogical_indicators
        ):
            score += 0.3
    ...
    def _determine_confidence(
        self, quality_score: float, llm_response: LLMResponse
    ) -> ConfidenceLevel:
