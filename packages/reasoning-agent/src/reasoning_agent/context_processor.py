...
class ContextProcessor:
    ...
    def __init__(self):
        ...
        self.subject_keywords = {
            SubjectArea.PHYSICS: [
                'force', 'velocity', 'acceleration', 'energy', 'momentum', 'gravity',
                'friction', 'pressure', 'wave', 'frequency', 'amplitude', 'newton',
                'kinematics', 'dynamics', 'thermodynamics', 'electricity', 'magnetism',
                'optics', 'quantum', 'relativity', 'mass', 'weight', 'motion'
            ],
            SubjectArea.CHEMISTRY: [
                'molecule', 'atom', 'bond', 'reaction', 'element', 'compound',
                'periodic', 'ionic', 'covalent', 'oxidation', 'reduction', 'catalyst',
                'acid', 'base', 'ph', 'mole', 'molarity', 'stoichiometry',
                'organic', 'inorganic', 'polymer', 'isomer', 'electron'
            ],
            SubjectArea.BIOLOGY: [
                'cell', 'organism', 'evolution', 'genetics', 'protein', 'dna',
                'rna', 'enzyme', 'photosynthesis', 'respiration', 'mitosis',
                'meiosis', 'ecosystem', 'species', 'habitat', 'adaptation',
                'mutation', 'chromosome', 'gene', 'allele', 'phenotype'
            ],
            SubjectArea.MATHEMATICS: [
                'equation', 'function', 'variable', 'graph', 'solution', 'theorem',
                'proof', 'derivative', 'integral', 'limit', 'matrix', 'vector',
                'polynomial', 'logarithm', 'trigonometry', 'geometry', 'algebra',
                'calculus', 'statistics', 'probability', 'coefficient'
            ],
            SubjectArea.HISTORY: [
                'century', 'civilization', 'empire', 'revolution', 'war', 'treaty',
                'democracy', 'monarchy', 'republic', 'constitution', 'independence',
                'colonial', 'ancient', 'medieval', 'renaissance', 'industrial',
                'primary source', 'artifact', 'chronology', 'era', 'dynasty'
            ],
            SubjectArea.LITERATURE: [
                'metaphor', 'symbolism', 'theme', 'character', 'plot', 'narrative',
                'poetry', 'prose', 'stanza', 'rhyme', 'meter', 'alliteration',
                'protagonist', 'antagonist', 'irony', 'foreshadowing', 'imagery',
                'allegory', 'satire', 'tragedy', 'comedy', 'genre'
            ],
            SubjectArea.SOCIAL_STUDIES: [
                'government', 'democracy', 'citizenship', 'economics', 'supply',
                'demand', 'market', 'capitalism', 'socialism', 'culture', 'society',
                'geography', 'climate', 'population', 'urban', 'rural',
                'globalization', 'migration', 'infrastructure', 'policy'
            ]
        }
    ...
    def extract_spatial_context(self, 
                               ...
                               surrounding_elements: List[Dict[str, Any]]) -> SpatialContext:
        ...
        sorted_elements = sorted(
            ...
            key=lambda el: (el.get('bounding_box', [0, 0, 0, 0])[1], el.get('bounding_box', [0, 0, 0, 0])[0])
        )
        ...
        for element in sorted_elements:
            ...
            # Collect headings
            if 'heading' in el_classification.lower() or 'h1' in element.get('html_tag', '').lower():
                nearby_headings.append(el_text)
                
                # Find containing section (closest preceding heading)
                if el_y < target_y and (not containing_section or el_y > target_y - 200):
    ...
    def detect_subject_area(self, 
                           context_text: str, 
                           page_metadata: Dict[str, Any]) -> SubjectDetectionResult:
        ...
        best_subject = max(subject_scores.keys(), key=lambda s: subject_scores[s]['score'])
        ...
        keyword_diversity = len(best_keywords) / len(self.subject_keywords[best_subject])
    ...
    def get_learning_context(self, 
                           ...
                           detected_subject: SubjectDetectionResult) -> Dict[str, Any]:
        ...
        elif detected_subject.area in [SubjectArea.HISTORY, SubjectArea.LITERATURE, SubjectArea.SOCIAL_STUDIES]:
