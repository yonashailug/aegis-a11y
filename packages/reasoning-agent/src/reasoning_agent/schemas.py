...
class ReasoningInput(BaseModel):
    ...
    surrounding_elements: list[dict[str, Any]] = Field(
        default_factory=list, description="Nearby elements for spatial context"
    )
    image_segment: bytes | None = Field(
        None, description="Image bytes of the specific element region"
    )
    full_page_image: bytes | None = Field(
        None, description="Full page image for broader context"
    )
    ...
    def from_cv_output(cls, extracted_element, **kwargs):
        ...
        element_dict = (
            extracted_element.model_dump()
            if hasattr(extracted_element, "model_dump")
            else extracted_element
        )
        return cls(extracted_element=element_dict, **kwargs)
...
class ReasoningOutput(BaseModel):
    ...
    subject_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in subject area detection"
    )

    # Learning context
    learning_objective: str | None = Field(
        None, description="Identified pedagogical purpose/objective"
    )
    ...
    pedagogical_alt_text: str = Field(
        ..., min_length=10, description="UDL-compliant educational description"
    )
    ...
    pedagogical_quality_score: float = Field(
        ..., ge=1.0, le=5.0, description="Self-assessed quality (1-5 scale from paper)"
    )
    ...
    llm_model_used: str = Field(default="gpt-4o", description="LLM model version used")
    ...
    processing_warnings: list[str] = Field(
        default_factory=list, description="Non-fatal issues during processing"
    )
    fallback_used: bool = Field(
        default=False, description="Whether fallback processing was used"
    )

    # Raw LLM response (for debugging)
    raw_llm_response: str | None = Field(
        None, description="Complete LLM response for debugging"
    )
...
class SpatialContext(BaseModel):
    ...
    containing_section: str | None = Field(None, description="Parent section title")
    nearby_headings: list[str] = Field(
        default_factory=list, description="Relevant headings"
    )
...
class LLMRequest(BaseModel):
    ...
    image_data: bytes | None = Field(
        None, description="Image for multimodal processing"
    )
    ...
    subject_hint: SubjectArea | None = Field(None, description="Subject area hint")
