...
from .quality_assessor import QualityMetrics, PedagogicalLevel
from .verifier import VerificationResult, ValidationIssue
...
class ReviewFeedback:
    ...
    # Detailed feedback
    pedagogical_accuracy: Optional[float] = None
    accessibility_compliance: Optional[float] = None
    subject_appropriateness: Optional[float] = None
    
    # Text feedback
    comments: str = ""
    suggested_improvements: List[str] = None
    corrected_alt_text: Optional[str] = None
...
class ReviewSession:
    ...
    final_action: Optional[ReviewAction] = None
    ...
    def __post_init__(self):
        ...
        if not self.session_id:
            self.session_id = f"review_{self.element_id}_{int(self.created_at.timestamp())}"
...
class HumanValidator:
    ...
    def __init__(self, callback_handler: Optional[Callable] = None):
        ...
        self.routing_rules = {
            'high_priority': lambda session: session.priority_level in ['high', 'critical'],
            'accessibility_expert': lambda session: session.verification_result.wcag_pass_rate < 0.8,
            'subject_expert': lambda session: session.quality_metrics.subject_relevance < 0.7,
            'escalation': lambda session: len(session.reviews) > 2 and not session.consensus_reached
        }
    ...
    def create_review_session(self,
                            ...
                            quality_metrics: QualityMetrics,
                            verification_result: VerificationResult,
                            priority_override: Optional[str] = None) -> ReviewSession:
        """
    ...
    def get_review_queue(self, 
                        reviewer_role: ReviewerRole,
                        subject_filter: Optional[SubjectArea] = None,
        ...
        for session in self.active_sessions.values():
            ...
            # Apply filters
            if subject_filter and session.original_output.detected_subject_area != subject_filter:
                continue
        ...
        queue.sort(key=lambda x: (priority_order.get(x['priority'], 2), x['created_at']))
    ...
    def get_review_interface_data(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'original_element': session.original_output.extracted_element if hasattr(session.original_output, 'extracted_element') else {},
            'ai_output': {
                'subject_area': session.original_output.detected_subject_area.value,
            ...
            'verification_results': {
                'wcag_pass_rate': session.verification_result.wcag_pass_rate,
                'issues': [asdict(issue) for issue in session.verification_result.issues],
            ...
            'review_guidelines': self._get_review_guidelines(session.original_output.detected_subject_area)
        }
    ...
    def generate_review_report(self, 
        ...
        if start_date:
            sessions = [s for s in sessions if s.completed_at and s.completed_at >= start_date]
        if end_date:
            sessions = [s for s in sessions if s.completed_at and s.completed_at <= end_date]
        ...
        avg_quality_score = sum(s.quality_metrics.pedagogical_score for s in sessions) / total_sessions
        avg_wcag_pass_rate = sum(s.verification_result.wcag_pass_rate for s in sessions) / total_sessions
        ...
        for session in sessions:
            for review in session.reviews:
                ...
                if role not in reviewer_stats:
                    reviewer_stats[role] = {'count': 0, 'avg_rating': 0, 'total_rating': 0}
                reviewer_stats[role]['count'] += 1
                reviewer_stats[role]['total_rating'] += review.quality_rating
                reviewer_stats[role]['avg_rating'] = reviewer_stats[role]['total_rating'] / reviewer_stats[role]['count']
        ...
        for session in sessions:
            ...
            if subject not in subject_stats:
                subject_stats[subject] = {'count': 0, 'avg_quality': 0, 'total_quality': 0}
            subject_stats[subject]['count'] += 1
            subject_stats[subject]['total_quality'] += session.quality_metrics.pedagogical_score
            subject_stats[subject]['avg_quality'] = subject_stats[subject]['total_quality'] / subject_stats[subject]['count']
        ...
        return {
            ...
            'quality_metrics': {
                ...
                'sessions_requiring_expert_review': len([s for s in sessions if s.requires_expert_review])
            },
    ...
    def _calculate_priority(self, 
        ...
        critical_issues = len([i for i in verification_result.issues if i.severity.value == 'fail'])
        if critical_issues > 2 or quality_metrics.pedagogical_score < 2.0:
            return 'critical'
        
        # High priority for significant quality issues
        if (quality_metrics.pedagogical_score < 3.0 or 
            verification_result.wcag_pass_rate < 0.7 or
            quality_metrics.overall_confidence < 0.6):
            return 'high'
        
        # Low priority for high-quality outputs
        if (quality_metrics.pedagogical_score >= 4.0 and
            verification_result.wcag_pass_rate >= 0.9 and
            quality_metrics.overall_confidence >= 0.8):
            return 'low'
        
        return 'normal'
    
    def _check_expert_review_required(self,
                                    quality_metrics: QualityMetrics,
                                    verification_result: VerificationResult) -> bool:
        """Check if expert review is required."""
        
        return (
            quality_metrics.pedagogical_score < 3.0 or
            verification_result.wcag_pass_rate < 0.8 or
            verification_result.requires_human_review or
            len([i for i in verification_result.issues if i.severity.value == 'fail']) > 1
        )
    ...
    def _check_consensus(self, session: ReviewSession) -> Dict[str, Any]:
        ...
        return {
            ...
            'next_reviewer_type': 'accessibility_specialist' if any(r.reviewer_role == ReviewerRole.EDUCATOR for r in session.reviews) else 'educator'
        }
    
    def _session_needs_reviewer(self, session: ReviewSession, reviewer_role: ReviewerRole) -> bool:
        ...
        if (reviewer_role == ReviewerRole.ACCESSIBILITY_SPECIALIST and
            reviewer_role not in existing_roles and
            session.verification_result.wcag_pass_rate < 0.9):
            return True
        
        # Administrator for escalated cases
        if (reviewer_role == ReviewerRole.ADMINISTRATOR and
            len(session.reviews) > 2 and not session.consensus_reached):
            return True
    ...
    def _identify_improvement_areas(self, sessions: List[ReviewSession]) -> List[Dict[str, Any]]:
        ...
        return [
            {'area': area, 'frequency': count, 'percentage': (count / len(sessions)) * 100}
