...
from .schemas import ConfidenceLevel, ReasoningOutput, SubjectArea
...
class DeterministicVerifier:
    ...
    def verify_reasoning_output(
        ...
        for criteria, validation_func in self.validation_rules.items():
            try:
                rule_issues = validation_func(
                    reasoning_output, original_element, spatial_context
                )
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        description=f"Validation rule failed: {e!s}",
        ...
        for issue in issues:
            if (
                issue.auto_fixable
                and corrections_applied < self.max_correction_attempts
            ):
                correction_result = self._apply_correction(
                    issue, corrected_output, original_element
                )
        ...
        overall_status = (
            ValidationResult.FAIL if has_failures else ValidationResult.PASS
        )

        # Calculate confidence score
        confidence_score = self._calculate_confidence_score(
            corrected_output, issues, wcag_pass_rate
        )

        # Determine if human review is required
        requires_human_review = (
            wcag_pass_rate < 0.8
            or corrected_output.pedagogical_quality_score
            < self.pedagogical_quality_threshold
            or corrected_output.confidence_level == ConfidenceLevel.LOW
        )
    ...
    def _validate_text_alternatives(
        self, output: ReasoningOutput, element: dict[str, Any], context: dict[str, Any]
        ...
        requires_alt_text = any(
            keyword in element_classification
            for keyword in ["figure", "image", "diagram", "chart", "graph"]
        )
    ...
    def _validate_reading_order(
        ...
        if spatial_context:
            ...
            if not containing_section and element.get("classification") not in [
                "heading",
                "title",
            ]:
                issues.append(
    ...
    def _validate_structure(
        ...
        if "table" in element_classification:
            ...
            has_structure_indicators = any(
                indicator in ocr_text.lower()
                for indicator in ["row", "column", "header", "cell", "|", "\t"]
            )
        ...
        # List validation
            ...
            has_list_indicators = any(
                indicator in ocr_text
                for indicator in ["•", "1.", "2.", "-", "*", "a)", "i)", "I."]
            )
        ...
        if issue.criteria == WCAGCriteria.TEXT_ALTERNATIVES:
            if "too short" in issue.description:
                ...
                if output.detected_subject_area != SubjectArea.UNKNOWN:
                    enhanced_alt_text += (
                        f". This {output.detected_subject_area.value} content"
                    )

                corrected_output.pedagogical_alt_text = enhanced_alt_text
                corrected_output.processing_warnings.append(
                    "Alt-text automatically expanded"
                )
                ...
                enhancement = subject_specific_phrases.get(
                    output.detected_subject_area, "supports learning objectives"
                )

                corrected_output.pedagogical_alt_text += f" that {enhancement}."
                corrected_output.processing_warnings.append(
                    "Alt-text enhanced with pedagogical focus"
                )

        # Mark that correction was applied
        corrected_output.fallback_used = True
        corrected_output.processing_warnings.append(
            f"Auto-correction applied for: {issue.description}"
        )
    ...
    def _calculate_confidence_score(
        ...
        critical_issues = len(
            [i for i in issues if i.severity == ValidationResult.FAIL]
        )
        critical_penalty = min(0.2, critical_issues * 0.05)

        confidence_score = (
            base_confidence + quality_confidence + wcag_confidence - critical_penalty
        )
    ...
    def generate_verification_report(
        ...
        return {
            ...
            "issues_by_severity": {
                "critical": [
                    i for i in result.issues if i.severity == ValidationResult.FAIL
                ],
                "warnings": [
                    i for i in result.issues if i.severity == ValidationResult.WARNING
                ],
            },
            "wcag_compliance": {
                ...
                "auto_fixable_issues": len(
                    [i for i in result.issues if i.auto_fixable]
                ),
            },
        }
