...
class AltTextGenerator:
    ...
    def generate_pedagogical_description(
        self,
        ...
        base_description = self._apply_subject_patterns(
            parsed_content, detected_subject, classification, element_text
        )

        # Apply UDL guidelines for accessibility
        enhanced_description = self._apply_udl_guidelines(
            base_description, detected_subject, spatial_context
        )

        # Generate rationale and importance
        rationale = self._generate_rationale(
            enhanced_description, detected_subject, spatial_context
        )

        importance = self._determine_importance(
            classification, spatial_context, detected_subject
        )
    ...
    def _parse_llm_response(self, llm_response: str) -> dict[str, str]:
        ...
        parsed = {"description": "", "educational_purpose": "", "key_concepts": ""}
    ...
    def _apply_subject_patterns(
        ...
        subject_config = self.subject_patterns.get(
            detected_subject.area, self.subject_patterns.get(SubjectArea.GENERAL, {})
        )
    ...
    def _enhance_physics_description(self, description: str, element_text: str) -> str:
        ...
        if any(word in element_text.lower() for word in ["graph", "chart", "plot"]):
            description += (
                " Focus on the relationship between variables and trends in the data."
            )

        return description

    def _enhance_chemistry_description(
        self, description: str, element_text: str
    ) -> str:
    ...
    def _apply_udl_guidelines(
        ...
        if spatial_context.containing_section:
            enhanced = (
                f"In the '{spatial_context.containing_section}' section: {enhanced}"
            )
    ...
    def _generate_rationale(
        ...
        if detected_subject.area in [
            ...
            rationale_parts.append(
                "Focuses on scientific concepts and relationships rather than visual appearance"
            )
        elif detected_subject.area == SubjectArea.MATHEMATICS:
            rationale_parts.append(
                "Emphasizes mathematical relationships and problem-solving context"
            )
        elif detected_subject.area in [SubjectArea.HISTORY, SubjectArea.LITERATURE]:
            rationale_parts.append(
                "Highlights educational significance and contextual meaning"
            )

        # Context-specific rationale
        if spatial_context.containing_section:
            rationale_parts.append(
                f"Connects to the learning objectives of the '{spatial_context.containing_section}' section"
            )

        # UDL rationale
        rationale_parts.append(
            "Provides equivalent access to visual information for screen reader users"
        )
    ...
    def _identify_applied_guidelines(self, description: str) -> list[str]:
        ...
        if any(
            ...
            for word in ["shows", "illustrates", "demonstrates"]
        ):
            applied.append("Alternative format for visual information (UDL 1.3)")

        if any(
            word in description.lower()
            for word in ["relationship", "pattern", "concept"]
        ):
