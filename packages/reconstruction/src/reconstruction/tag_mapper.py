...
from .schemas import DocumentStructure, ReconstructionInput, TagMapping
...
class JSONToTagMapper:
    ...
    def __init__(self, tag_mapping: TagMapping | None = None):
    ...
    def map_to_document_structure(
        ...
        layout_context = self._analyze_layout(
            verified_elements, reconstruction_input.original_layout
        )
        ...
        document_sections = self._create_semantic_sections(
            structured_elements, layout_context
        )
    ...
    def _analyze_layout(
        self, elements: list[ReasoningOutput], original_layout: list[dict[str, Any]]
    ) -> LayoutContext:
        ...
        for element in elements:
            ...
            bbox = None
            if (
                hasattr(element, "extracted_element")
                and "bounding_box" in element.extracted_element
            ):
            ...
            if not bbox and original_layout:
                for layout_item in original_layout:
                    if layout_item.get("bbox") and (
                        element.element_id in str(layout_item)
                        or element.detected_subject_area.value in str(layout_item)
                    ):
        ...
        page_width = max(
            (pos["x"] + pos["width"] for pos in element_positions.values()), default=800
        )
        page_height = max(
            (pos["y"] + pos["height"] for pos in element_positions.values()),
            default=1200,
        )
        ...
        reading_order = self._determine_reading_order(
            list(element_positions.values()), column_count
        )
    ...
    def _determine_reading_order(
        self, positions: list[dict[str, Any]], column_count: int
    ) -> list[str]:
        ...
        if column_count == 1:
            ...
            sorted_positions = self._sort_multicolumn_reading_order(
                positions, column_count
            )

        return [pos["element"].element_id for pos in sorted_positions]

    def _sort_multicolumn_reading_order(
        self, positions: list[dict[str, Any]], column_count: int
    ) -> list[dict[str, Any]]:
        ...
        column_width = (
            max(x_positions) / column_count if column_count > 1 else float("inf")
        )
    ...
    def _identify_sections(
        self, elements: list[ReasoningOutput], positions: dict[str, Any]
    ) -> list[dict[str, Any]]:
        ...
        for element in elements:
            ...
            if (
                hasattr(element, "extracted_element")
                and "classification" in element.extracted_element
            ):
    ...
    def _process_elements_in_order(
        self, elements: list[ReasoningOutput], layout_context: LayoutContext
    ...
    def _convert_element_to_structure(
        self, element: ReasoningOutput
    ) -> DocumentStructure:
        ...
        if (
            hasattr(element, "extracted_element")
            and "ocr_text" in element.extracted_element
        ):
        ...
        return DocumentStructure(
            ...
            aria_label=(
                element.pedagogical_alt_text
                if classification in ["functional_diagram", "equation"]
                else None
            ),
    ...
    def _build_element_attributes(
        self, element: ReasoningOutput, classification: str
    ) -> dict[str, str]:
        ...
        if element.pedagogical_alt_text and classification in [
            "functional_diagram",
            "equation",
        ]:
    ...
    def _create_semantic_sections(
        ...
        for element in structured_elements:
            ...
            if (
                element.element_type.startswith("h")
                and element.element_type[1:].isdigit()
            ):
        ...
        if sections:
            main_content = DocumentStructure(
                ...
                attributes={"role": "main", "class": "document-main"},
                children=sections,
            )
            return [main_content]
    ...
    def _collect_headings(
        self, structure: DocumentStructure, headings: list[DocumentStructure]
    ):
    ...
    def _validate_alt_text_recursive(self, structure: DocumentStructure):
        ...
        needs_alt_text = (
            structure.element_type in ["img", "figure"]
            or "diagram" in structure.element_type
        )

        if (
            needs_alt_text
            and not structure.alt_text
            and not structure.attributes.get("aria-label")
        ):
