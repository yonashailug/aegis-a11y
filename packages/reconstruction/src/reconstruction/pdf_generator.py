...
class PDFUAGenerator:
    ...
    def generate_pdf_ua_document(self, 
                                document_structure: DocumentStructure,
                                reconstruction_input: ReconstructionInput) -> bytes:
        """
        ...
        doc.build(story, onFirstPage=self._add_pdf_ua_metadata,
    ...
    def _build_title_page(self, reconstruction_input: ReconstructionInput) -> List[Any]:
        ...
        # Main title
        title = Paragraph(
            reconstruction_input.document_title,
            self.styles['AccessibleHeading1']
        )
    ...
    def _convert_structure_to_pdf_elements(self, structure: DocumentStructure) -> List[Any]:
        """Convert document structure to PDF elements."""
        elements = []
        ...
        if structure.element_type == "html":
            ...
            for child in structure.children:
                elements.extend(self._convert_structure_to_pdf_elements(child))
        elif structure.element_type.startswith('h') and structure.element_type[1:].isdigit():
            # Heading
